...
```{r}
gg<- ggplot(data = vaccine_overall[vacyear >= 2017 & vacyear <= 2020], aes(x = factor(vacmonth), y = N/1000, color = factor(vacyear))) +
...
```
```{r}
ggplot(data = vaccine_delay[vacyear >= 2017 & vacyear <= 2020], aes(x = factor(vacmonth), y=vacdelay, color = factor(vacyear))) +
  geom_line() +
  facet_wrap(~ vacyear) +
  theme_bw()
...
mea <- measle[, .N, by = .(vacyear, vacmonth, vacdelay, vacname2)]
mea2 <- measle[, .N, by = .(vacyear, vacmonth, vacweek, vacdelay, vacname2)]
```
