...
gg + stat_smooth(method="loess", formula = y ~ x, colour="blue", size=1.5)
>>>>>>> 364071a3848d3fc5813c5da46cefbc4397b35dcb
```
...
=======
ggplot(data = mea[vacyear >= 2017 & vacyear <= 2020], aes(x = factor(vacmonth), y = vacdelay, color = factor(vacyear))) +
  geom_point() +
  facet_grid(vacname2~ vacyear) +
  #geom_rect(aes(xmin = 3, xmax = 5, ymin = -Inf, ymax = Inf), fill = "grey80", color = "grey90", alpha = 0.015) +
  #geom_line(aes(group = vacyear), size = 1.5) +
  scale_x_discrete(name = "Month") +
  scale_y_continuous(name = "Number of shot (x 10^3)", breaks = seq(from = -20, to = 60, by = 20)) +
>>>>>>> 364071a3848d3fc5813c5da46cefbc4397b35dcb
  scale_color_discrete(name = "Year") +
  theme_bw() +
