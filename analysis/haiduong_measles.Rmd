...
```{r}
vaccov <- function(vcnames, start_at, months_follow, yrs) {
...
```{r}
df_cov <- vaccov(vcnames = c("Measles"), 
                 start_at = 8, 
                 months_follow = 13,
...
df_plot <- df_cov[which(year(df_cov$cohort) == yr),]
...
```{r}
cutoff_month <- c(10, 12, 14, 16, 18, 20)

df_plot <- df_cov %>% filter(n_month %in% cutoff_month)

# Format to display month as Jan, Feb
df_plot$cohort_month <- format(df_plot$cohort, format = "%b")
# Set correct order (J > F > M > A...)
df_plot$cohort_month <- factor(df_plot$cohort_month, levels = month.abb)

df_plot$cohort_year <- format(df_plot$cohort, format = "%Y")
df_plot$cohort_year <- factor(df_plot$cohort_year)

ggplot(df_plot, aes(x = cohort_month, y = cov, group = cohort_year)) +
  geom_point(aes(shape = cohort_year, color = cohort_year)) +
  ylim(0, 1.2) +
...
```{r}
df_cov <- vaccov(vcnames = c("Measles", "MR"), 
                 start_at = 8, 
                 months_follow = 13,
                 yrs = c(2017, 2018, 2019, 2020))
```
...
```{r}
df_cov <- vaccov(vcnames = c("Measles", "MR", "MMR"), 
                 start_at = 8, 
                 months_follow = 13,
                 yrs = c(2017, 2018, 2019, 2020))
```
