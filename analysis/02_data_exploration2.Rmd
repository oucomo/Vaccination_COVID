...
vaccine_vad$vsche <- as.Date(vaccine_vad$vsche)
vaccine_vad$vsyear <- year(vaccine_vad$vsche)
vaccine_vad$vsmonth <- month(vaccine_vad$vsche)
vaccine_vad$vsweek <- week(vaccine_vad$vsche)

vaccine_overall <- vaccine_vad[, .N, by = .(vyear, vmonth)]
vaccine_overall2 <- vaccine_vad[, .N, by = .(vyear, vmonth, vweek)]

vaccine_schedule <- vaccine_vad[, .N, by = .(vsyear, vsmonth)]
vaccine_schedule2 <- vaccine_vad[, .N, by = .(vsyear, vsmonth, vsweek)]

vaccine_province <- vaccine_vad[, .N, by = .(vyear, vmonth, province)]
vaccine_province2 <- vaccine_vad[, .N, by = .(vyear, vmonth, vweek, province)]
vaccine_vacname <- vaccine_vad[, .N, by = .(vyear, vmonth, vacname2)]
vaccine_vacname2 <- vaccine_vad[, .N, by = .(vyear, vmonth, vweek, vacname2)]
vaccine_province_vacname <- vaccine_vad[, .N, by = .(vyear, vmonth, province, vacname2)]
vaccine_province_vacname2 <- vaccine_vad[, .N, by = .(vyear, vmonth, vweek, province, vacname2)]
...
tab_vac_sum <- tbl_summary(vaccine_vad[vyear >= 2017 & vyear <= 2020, .(province, vacname2, factor(vyear))])
...
#ggsave(filename = file.path("..", "figures", "child_month_year.png"), width = 7, height = 5)
```
...
```{r}
ggplot(vaccine_overall[vyear >= 2017 & vyear <= 2020], aes(vmonth, N/1000, color = factor(vyear))) +
  geom_point(size=3) +
...
```{r}
ggplot(data = vaccine_overall2[vyear >= 2017 & vyear <= 2020], aes(x = factor(vweek), y = N/1000, color = factor(vyear))) +
  #geom_rect(aes(xmin = 3, xmax = 5, ymin = -Inf, ymax = Inf), fill = "grey80", color = "grey90", alpha = 0.015) +
  geom_point(size = 3) +
  geom_line(aes(group = vyear), size = 1.5) +
  scale_x_discrete(name = "Week") +
...
```{r}
ggplot(vaccine_overall2[vyear >= 2017 & vyear <= 2020], aes(vweek, N/1000, color = factor(vyear))) +
  geom_point(size=3) +
  geom_line(aes(group=vyear)) +
  geom_smooth () + 
  stat_smooth(method="loess", formula = y ~ x, colour="blue", size=1) +
...
```{r}
ggplot(data = vaccine_vacname[vyear >= 2017 & vyear <= 2020], aes(x = factor(vmonth), y = N/1000)) +
  geom_rect(aes(xmin = 3, xmax = 5, ymin = -Inf, ymax = Inf), fill = "grey80", color = "grey90", alpha = 0.015) +
...
#mea <- measle[, .N, by = .(vyear, vmonth)]
#mea2 <- measle[, .N, by = .(vyear, vmonth, vweek)]
#mea_delay <- measle[, .N, by = .(vyear, vmonth, vdelayd, vacname2)]
```

#### Overall

```{r}
ggplot(data = mea[vyear >= 2017 & vyear <= 2020], aes(x = factor(vmonth), y = N/1000, color = factor(vyear))) +
  geom_rect(aes(xmin = 3, xmax = 5, ymin = -Inf, ymax = Inf), fill = "grey80", color = "grey90", alpha = 0.015) +
  geom_point(size = 3) +
  geom_line(aes(group = vyear), size = 1.5) +
  scale_x_discrete(name = "Month") +
...
```{r}
ggplot(mea[vyear >= 2017 & vyear <= 2020], aes(vmonth, N/1000, color = factor(vyear))) +
  geom_point(size=3) +
  geom_line(aes(group=vyear)) +
  geom_smooth () + 
  stat_smooth(method="loess", formula = y ~ x, colour="blue", size=1) +
...
```{r}
ggplot(data = mea2[vyear >= 2017 & vyear <= 2020], aes(x = factor(vweek), y = N/1000, color = factor(vyear))) +
  #geom_rect(aes(xmin = 3, xmax = 5, ymin = -Inf, ymax = Inf), fill = "grey80", color = "grey90", alpha = 0.015) +
  geom_point(size = 3) +
  geom_line(aes(group = vyear), size = 1.5) +
  scale_x_discrete(name = "Week") +
...
```{r}
ggplot(mea2[vyear >= 2017 & vyear <= 2020], aes(vweek, N/1000, color = factor(vyear))) +
  geom_point(size=3) +
  geom_line(aes(group=vyear)) +
  geom_smooth () + 
  stat_smooth(method="loess", formula = y ~ x, colour="blue", size=1) +
...
ggplot(data = measle[vyear >= 2017 & vyear <= 2020 & vdelayd>=0], aes(x = factor(vmonth), y = vdelayd, color = factor(vyear))) +
  geom_point() +
...
```{r}
ggplot(data = measle[vyear >= 2017 & vyear <= 2020 & vdelayd>=0], aes(x = vdelayd, y = factor(vmonth), color = factor(vyear))) +
  geom_density_ridges() +
  theme_ridges() + 
...
```{r}
ggplot(data = measle[vsyear >= 2017 & vsyear <= 2020 & vdelay>=0], aes(x = vdelay, y = factor(vsmonth), color = factor(vsyear))) +
  geom_density_ridges() +
  theme_ridges() + 
  theme(legend.position = "none") +
...
```{r}
ggplot(data = measle[year>= 2017 & year <= 2020 & vdelay>=0], aes(x = vdelay, y = factor(month), color = factor(year))) +
  geom_density_ridges() +
  theme_ridges() + 
  theme(legend.position = "none") +
...
measle_delay <- measle %>%
  select(pid, province, district, commune, sex, dob, vacname2, epi, msl, start, vacdate, vyear, vmonth, year, month, vagem, vagem2, vdelay, vsche, vsyear, vsmonth, vdelayd) %>% filter(year>=2017 & year<=2021) %>%
...
```{r}
t1 <- measle_delay %>% select(age, sex, province, vacname2, vdelay, vdelay_outcome) %>% 
  tbl_summary(
...
library("rms")
theme_set(theme_bw())
dd <- datadist(measle_delay)
options(datadist="dd")
dd

fit1 <- lrm(vdelay_outcome~age, data=measle_delay)
anova(fit1)
...
```{r}
fit2 <- lrm(vdelay_outcome~sex, data=measle_delay)
anova(fit2)
...
```{r}
fit3 <- lrm(vdelay_outcome~province, data=measle_delay)
anova(fit3)
...
```{r}
fit4 <- lrm(vdelay_outcome~vacname2, data=measle_delay)
anova(fit4)
...
```{r}
fit5 <- lrm(vdelay_outcome~vmonth, data=measle_delay)
anova(fit5)
...
```{r}
measle_model <- measle_delay %>% select (vdelay_outcome, sex, age, province)
model <- glm(vdelay_outcome~., family = binomial(link = "logit"), data=measle_model)
summary(model)
...
measle_model <- na.omit(measle_model)
mydata <- measle_model %>%
...
```{r, eval=FALSE, echo=FALSE}
ggplot(data = mea_delay[vyear >= 2017 & vyear <= 2020], aes(vdelay, color = factor(vyear))) +
