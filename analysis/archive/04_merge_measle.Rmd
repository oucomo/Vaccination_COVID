...
#save(child_all_overall, child_all_overall2, child_all_province, child_all_province2, child_all_sex, child_all_sex2, child_all_province_sex, child_all_province_sex2, file = file.path("..", "..", "tuann349_vad", "child_all.Rdata"))

tab_child_all_sum <- tbl_summary(child_all[year >= 2017 & year <= 2020, .(province, sex, age, type, type2, factor(year))])

#save(tab_child_all_sum, file = file.path("..", "..", "tuann349_vad", "tab_child_all.Rdata"))
```
...
#ggsave(filename = file.path("..", "figures", "child_all_month_year.png"), width = 7, height = 5)
```
...
ggplot(child_all_overall[year >= 2017 & year <= 2020], aes(month, N/1000, color = factor(year))) +
  ...
  theme_bw() +
  ggtitle("Total number of vaccination children born by month and year")
#ggsave(filename = file.path("..", "figures", "child_all_month_year2.png"), width = 7, height = 5)
```
...
#ggsave(filename = file.path("..", "figures", "child_all_province_year.png"), width = 10, height = 7)
```
...
measle_public <- measle_merge %>% filter(type=="public") %>% 
    select(pid, province.x, district.x, commune.x, sex.x, dob.x, vacname2.x, vacdate.x, shot.x, start.x, vyear.x, vmonth.x,vweek.x,vagem.x, vagem2.x, vdelay.x, vsche.x, vsyear.x, vsmonth.x, vsweek.x, vdelayd.x, type) %>% 
    arrange(pid, vacdate.x, vacname2.x) %>% 
    rename(province=province.x, district=district.x, commune=commune.x, sex=sex.x, dob=dob.x, vacname2=vacname2.x, vacdate=vacdate.x, shot=shot.x, start=start.x, vyear=vyear.x, vmonth=vmonth.x, vweek=vweek.x, vagem=vagem.x, vagem2=vagem2.x, vdelay=vdelay.x, vsche=vsche.x, vsyear=vsyear.x, vsmonth=vsmonth.x, vsweek=vsweek.x, vdelayd=vdelayd.x)

measle_private <- measle_merge %>% filter(type=="private") %>% 
    select(pid, province.y, district.y, commune.y, sex.y, dob.y, vacname2.y, vacdate.y, shot.y, start.y, vyear.y, vmonth.y,vweek.y,vagem.y, vagem2.y, vdelay.y, vsche.y, vsyear.y, vsmonth.y, vsweek.y, vdelayd.y, type) %>% 
    arrange(pid, vacdate.y, vacname2.y) %>%
    rename(province=province.y, district=district.y, commune=commune.y, sex=sex.y, dob=dob.y, vacname2=vacname2.y, vacdate=vacdate.y, shot=shot.y, start=start.y, vyear=vyear.y, vmonth=vmonth.y, vweek=vweek.y, vagem=vagem.y, vagem2=vagem2.y, vdelay=vdelay.y, vsche=vsche.y, vsyear=vsyear.y, vsmonth=vsmonth.y, vsweek=vsweek.y, vdelayd=vdelayd.y)

measle_both_public <- measle_merge %>% filter(type=="both") %>%
  select(pid, province.x, district.x, commune.x, sex.x, dob.x, vacname2.x, vacdate.x, shot.x, start.x, vyear.x, vmonth.x,vweek.x,vagem.x, vagem2.x, vdelay.x, vsche.x, vsyear.x, vsmonth.x, vsweek.x, vdelayd.x, type) %>%
    arrange(pid, vacdate.x, vacname2.x) %>%
    rename(province=province.x, district=district.x, commune=commune.x, sex=sex.x, dob=dob.x, vacname2=vacname2.x, vacdate=vacdate.x, shot=shot.x, start=start.x, vyear=vyear.x, vmonth=vmonth.x, vweek=vweek.x, vagem=vagem.x, vagem2=vagem2.x, vdelay=vdelay.x, vsche=vsche.x, vsyear=vsyear.x, vsmonth=vsmonth.x, vsweek=vsweek.x, vdelayd=vdelayd.x) %>% mutate(type2="public")
...
measle_both <- rbind(measle_both_public, measle_both_private) %>% arrange(pid, vacdate, vacname2)
...
#ggsave(filename = file.path("..", "figures", "measle_all_overall_month_year.png"), width = 7, height = 5)
```
...
ggplot(data = mea_all[vyear >= 2017 & vyear <= 2020], aes(x = factor(vmonth), y = N/1000, color = factor(vyear))) +
  ...
  facet_wrap(~ type) +
  ggtitle("Total number of Measle vaccination shots by month and year")
#ggsave(filename = file.path("..", "figures", "measle_all_month_year.png"), width = 7, height = 5)
```
...
#ggsave(filename = file.path("..", "figures", "measle_all_month_year2.png"), width = 7, height = 5)
```
...
#ggsave(filename = file.path("..", "figures", "measle_all_delay_year.png"), width = 7, height = 5)
```
...
#ggsave(filename = file.path("..", "figures", "measle_all_delay_age.png"), width = 7, height = 5)
```
...
ggplot(pred, ylab=expression(paste(hat(P),"(delay)"))) +
  ...
  geom_ribbon(aes(ymin = pred$lower, ymax = pred$upper), alpha=0.05) + 
  xlab("Province")
#ggsave(filename = file.path("..", "figures", "measle_all_delay_province.png"), width = 7, height = 5)
```
...
#ggsave(filename = file.path("..", "figures", "measle_all_delay_period.png"), width = 7, height = 5)
```
...
#ggsave(filename = file.path("..", "figures", "measle_all_delay_type.png"), width = 7, height = 5)
```
