...
datap <- file.path("/cluster_data", "vaccine_registry", "DVD1", "107.HaiDuong")

# Load data
df <- readRDS(file.path(datap, "haiduong_14_22_merged.rds"))
```
...
df_vc <- as.data.table(
  rbind(
    ...
    cbind(vacname = "BCG-TCDV",                vacname2 = "BCG"),
    ...
    # Hep B
    cbind(vacname = "Viem gan B",              vacname2 = "HepB"),
    cbind(vacname = "Viem gan B so sinh",      vacname2 = "HepB"),
    cbind(vacname = "Heberbiovac HB 10mcg/0,5ml", vacname2 = "HepB"),
    cbind(vacname = "Heberbiovac HB 20mcg/1ml", vacname2 = "HepB"),
    cbind(vacname = "Hepavax-Gene TF(R) Inj 10mcg/0,5ml", vacname2 = "HepB"),
    cbind(vacname = "Gene-HBvax (Lo 0.5ml)",   vacname2 = "HepB"),
    cbind(vacname = "Gene-HBvax (Lo 1ml)",     vacname2 = "HepB"),
    cbind(vacname = "Euvax B 10mcg/0,5ml",     vacname2 = "HepB"),
    cbind(vacname = "Euvax B  20mcg/1ml",      vacname2 = "HepB"),
    cbind(vacname = "ENGERIX-B 10mcg/0,5ml",   vacname2 = "HepB"),
    cbind(vacname = "ENGERIX-B 20mcg/1ml",     vacname2 = "HepB"),
...
vacinfo <- as.data.table(
  rbind(
    ...
    cbind(vacname2 = "HepB",          epi = 1, shot = 1, start = 0),
...
cols_order <- c(cols_rn, "vacname", "vacname2", "shot", "vacyear", "vacmonth", "epi",
                "start", "vagem", "vdelay")
df <- df[,cols_order]
```
...
```{r}
bcg <- df[which(df$vacname2 == "BCG"),]
saveRDS(bcg, file.path(datap, "bcg_haiduong.rds"))
```
...
```{r}
hepb <- df[which(df$vacname2 == "HepB"),]
saveRDS(hepb, file.path(datap, "hepb_haiduong.rds"))
```
...
# saveRDS(measles, file.path(datap, "measles_haiduong.rds"))
```
