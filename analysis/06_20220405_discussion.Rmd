...
measle_all$expect_vdate <- measle_all$dob %m+% months(measle_all$start2)
```
...
```{r}
df <- measles_2shots[, c("pid", "province", "vacdate_1st", "vtimes", "type2")]
df <- df %>%
  pivot_wider(., names_from = vtimes, values_from = type2)
...
df_type <- aggregate(pid ~ vyear_1st + vmonth_1st + province + shot1 + shot2, data = df, FUN = length)

df_type <- df_type %>%
  group_by(vyear_1st, province, vmonth_1st, shot1) %>%
  mutate(denom = sum(pid))
...
# An example
df_type %>%
  filter(vyear_1st == 2018 & province == "Ha Noi" & vmonth_1st == 1) %>%
  print()
```
...
ggplot(df_plot, aes(x = vacdate_1st, y = pct2, group = shot1)) +
  geom_line(aes(linetype = shot1), stat = "identity") +
  geom_vline(xintercept=as.numeric(as.Date("2019-07-01")), color = "orange") +
  facet_wrap(. ~ province) +
  ylim(c(0, 100)) +
  theme_minimal() +
...
## Population level
```{r}
df <- measle_all[, c("pid", "province", "vyear", "vmonth", "type2")]
df_type <- aggregate(pid ~ vyear + vmonth + type2, data = df, FUN = length)

# Percentage of private
df_type <- df_type %>%
  group_by(vyear, vmonth) %>%
  mutate(denom = sum(pid))
...
# To plot on a date format x-axis
df_plot$vacdate <- ym(paste0(df_plot$vyear, "-", df_plot$vmonth))
...
A sudden increase in percentage of private shot in April because there were less shots in public (the denominator is low)
```{r}
df_plot <- data.frame(df_plot)
df_plot[order(df_plot$vacdate),]
```

## Plot per province
```{r}
df_type <- aggregate(pid ~ vyear + province + vmonth + type2, data = df, FUN = length)

# Percentage of private
df_type <- df_type %>%
  group_by(vyear, province, vmonth) %>%
  mutate(denom = sum(pid))
...
ggplot(df_plot, aes(x = vacdate, y = pct)) +
  ...
  geom_vline(xintercept=as.numeric(as.Date("2020-04-01")), color = "orange") +
  facet_wrap(. ~ province) +
  labs(x = "Percentage of private shot", y = "Month of receiving shot") +
  theme_light()
```

## Missing measles shot for children born in 2019 and 2020: vaccine Measles (at 9 month)
```{r}
...
child <- hepatitisb_all[hepatitisb_all$province %in% publicProvinces &
                        hepatitisb_all$year == 2019 |
                        (hepatitisb_all$year == 2020 & hepatitisb_all$month <= 3),]


measles_1920 <- measle_all[measle_all$province %in% publicProvinces & 
                           measle_all$year == 2019 | (measle_all$year == 2020 & measle_all$month <= 3),]
id_measles <- unique(measles_1920$pid)
id_hepb <- unique(child$pid[child$vacname2 %in% c("HepB", "HepBN")])

length(intersect(id_measles, id_hepb))
```
