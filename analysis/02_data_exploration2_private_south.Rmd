...
t1 <- vaccine_vad_private_south %>% 
  mutate(age=2021-as.numeric(year),
         agegr=NA,
         agegr=case_when(age>=0 & age<=10 ~ 1,
                         age>10 & age<=20 ~ 2,
                         age>20 & age<=30 ~ 3,
                         age>30 & age<=40 ~ 4,
                         age>40 & age<=50 ~ 5,
         ...
         yeargr=NA,
         yeargr=case_when(year>=1969 & year<=1980 ~ 1,
                         year>1980 & year<=1990 ~ 2,
                         year>1990 & year<=2000 ~ 3,
                         year>2000 & year<=2010 ~ 4,
                         year>2010 & year<=2020 ~ 5,
                         TRUE ~ yeargr),
         yeargr=factor(yeargr, levels = c(1,2,3,4,5), labels = c("1969-1980", "1981-1990", "1991-2000", "2001-2010","2011-2020"))) %>%
  dplyr::select(age, agegr, sex, province, type, type2, yeargr) %>%
  tbl_summary(
...
```{r}
ggplot(data = individual_private_south_overall, aes(x = factor(month), y = N/1000, color = factor(yeargr))) +
...
```{r}
ggplot(individual_private_south_overall, aes(month, N/1000, color = factor(yeargr))) +
  geom_point(size=2) +
