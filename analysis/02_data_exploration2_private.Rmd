...
vaccine_vad_private$vweek <- week(vaccine_vad_private$vacdate)
vaccine_vad_private$vsyear <- year(vaccine_vad_private$vsche)
vaccine_vad_private$vsmonth <- month(vaccine_vad_private$vsche)
vaccine_vad_private$vsweek <- week(vaccine_vad_private$vsche)
...
```{r}
ggplot(data = vaccine_overall_private[vyear >= 2017 & vyear <= 2020], aes(x = factor(vmonth), y = N/1000, color = factor(vyear))) +
...
```{r}
ggplot(vaccine_overall_private[vyear >= 2017 & vyear <= 2020], aes(vmonth, N/1000, color = factor(vyear))) +
  geom_point(size=3) +
...
```{r}
ggplot(data = vaccine_province_private[vyear >= 2017 & vyear <= 2020], aes(x = factor(vmonth), y = N/1000, color = factor(vyear))) +
  geom_rect(aes(xmin = 3, xmax = 5, ymin = -Inf, ymax = Inf), fill = "grey80", color = "grey90", alpha = 0.015) +
  geom_point() +
  geom_line(aes(group = vyear)) +
  scale_x_discrete(name = "Month") +
...
```{r}
ggplot(data = measle[byear>= 2017 & byear <= 2020 & vdelay>=0], aes(x = vdelay, y = factor(bmonth), color = factor(byear))) +
  geom_density_ridges() +
  theme_ridges() + 
  theme(legend.position = "none") +
